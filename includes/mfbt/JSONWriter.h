...
public:
  ...
  template <size_t N> void NullProperty(const char (&aName)[N]) {
  ...
  template <size_t N> void BoolProperty(const char (&aName)[N], bool aBool) {
    // Keep null terminator from literal strings, will be removed by
    // EscapedString. This way C buffer arrays can be used as well.
  ...
  template <size_t N> void IntProperty(const char (&aName)[N], int64_t aInt) {
    // Keep null terminator from literal strings, will be removed by
    // EscapedString. This way C buffer arrays can be used as well.
  ...
  template <size_t N> void StringElement(const char (&aName)[N]) {
    // Keep null terminator from literal strings, will be removed by
    // EscapedString. This way C buffer arrays can be used as well.
