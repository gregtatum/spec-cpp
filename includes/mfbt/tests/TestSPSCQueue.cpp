...
template <typename T> class SequenceGenerator {
public:
...
template <typename T> class SequenceVerifier {
public:
...
template <typename T> void TestRing(int capacity) {
...
template <typename T> void TestRingMultiThread(int capacity) {
  SPSCQueue<T> buf(capacity);
  SequenceVerifier<T> checker;
...
template <typename T> void BasicAPITest(T &ring) {
