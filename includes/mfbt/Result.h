...
template <typename V> struct EmptyWrapper : V {
...
template <typename V, typename E> class ResultImplementationNullIsOkBase {
protected:
...
public:
  ...
  operator=(ResultImplementationNullIsOkBase &&aOther) {
    if constexpr (!std::is_empty_v<V>) {
      if (isOk()) {
        mValue.first().addr()->~V();
      }
...
public:
  ...
  operator=(ResultImplementationNullIsOk &&) = default;
...
template <typename V, typename E> struct IsPackableVariant {
  struct VEbool {
    V v;
    E e;
    bool ok;
...
// }  // namespace mozilla::detail
//
template <typename T> struct UnusedZeroEnum {
...
// empty.
template <typename T> struct HasFreeLSB {
  static const bool value = std::is_empty_v<T>;
};

// As an incomplete type, void* does not have a spare bit.
template <> struct HasFreeLSB<void *> { static const bool value = false; };
...
// the V and E types.
template <typename V, typename E> struct SelectResultImpl {
 ...
 */
template <typename V, typename E> class MOZ_MUST_USE_TYPE Result final {
...
public:
   ...
   */
  template <typename F> constexpr auto mapErr(F f) {
 ...
 */
template <typename E> class MOZ_MUST_USE_TYPE GenericErrorResult {
  E mErrorValue;
...
template <typename E> inline constexpr auto Err(E &&aErrorValue) {
