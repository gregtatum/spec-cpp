...
template <class T, class U> inline constexpr T narrow_cast(U &&u) {
...
template <class SpanT, bool IsConst> class span_iterator {
...
public:
  ...
  constexpr span_iterator<SpanT, IsConst> &
  operator=(const span_iterator<SpanT, IsConst> &) = default;
...
template <size_t Ext> class extent_type {
public:
...
public:
   ...
   */
  template <size_t N>
  constexpr MOZ_IMPLICIT
  Span(const std::array<std::remove_const_t<element_type>, N> &aArr)
      : storage_(&aArr[0], span_details::extent_type<N>()) {}
  ...
  template <size_t N>
  constexpr MOZ_IMPLICIT
  Span(const mozilla::Array<std::remove_const_t<element_type>, N> &aArr)
      : storage_(&aArr[0], span_details::extent_type<N>()) {}
  ...
  Subspan(index_type aStart, index_type aLength = dynamic_extent) const {
    const size_t len = size();
...
template <typename T, size_t Extent> Span(T (&)[Extent]) -> Span<T, Extent>;
