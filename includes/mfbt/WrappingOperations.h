...
template <typename UnsignedType> struct WrapToSignedHelper {
  ...
  MOZ_NO_SANITIZE_SIGNED_OVERFLOW static constexpr SignedType
  compute(UnsignedType aValue) {
...
template <typename T> struct WrappingSubtractHelper {
private:
  using UnsignedT = std::make_unsigned_t<T>;
 ...
 * permissibly -- triggers different behavior.
 */
template <typename T> constexpr T WrappingSubtract(T aX, T aY) {
  return detail::WrappingSubtractHelper<T>::compute(aX, aY);
}

namespace detail {

template <typename T> struct WrappingMultiplyHelper {
private:
  using UnsignedT = std::make_unsigned_t<T>;
 ...
 */
template <typename T> constexpr T WrappingMultiply(T aX, T aY) {
