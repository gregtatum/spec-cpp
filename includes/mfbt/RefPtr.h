...
template <class T> class StaticLocalRefPtr;
...
template <class T> class MOZ_IS_REFPTR RefPtr {
private:
...
public:
  ...
  */
  ...
  template <typename R, typename... Args> class Proxy {
  ...
  public:
    ...
    template <typename... ActualArgs> R operator()(ActualArgs &&... aArgs) {
...
private:
  ...
  template <class U> struct ConstRemovingRefPtrTraits {
...
template <typename T> inline void ImplCycleCollectionUnlink(RefPtr<T> &aField) {
  aField = nullptr;
}
...
ImplCycleCollectionTraverse(nsCycleCollectionTraversalCallback &aCallback,
                            RefPtr<T> &aField, const char *aName,
                            uint32_t aFlags = 0) {
  CycleCollectionNoteChild(aCallback, aField.get(), aName, aFlags);
}

template <class T> inline RefPtr<T> *address_of(RefPtr<T> &aPtr) {
  return aPtr.get_address();
}

template <class T> inline const RefPtr<T> *address_of(const RefPtr<T> &aPtr) {
  return aPtr.get_address();
}
...
inline bool operator==(const RefPtr<T> &aLhs, const RefPtr<U> &aRhs) {
  return static_cast<const T *>(aLhs.get()) ==
         static_cast<const U *>(aRhs.get());
}
...
template <class T> inline already_AddRefed<T> do_AddRef(T *aObj) {
  RefPtr<T> ref(aObj);
  return ref.forget();
}

template <class T> inline already_AddRefed<T> do_AddRef(const RefPtr<T> &aObj) {
  RefPtr<T> ref(aObj);
  return ref.forget();
}
 ...
 */
template <typename T, typename... Args> RefPtr<T> MakeRefPtr(Args &&... aArgs) {
  RefPtr<T> p(new T(std::forward<Args>(aArgs)...));
  return p;
}
