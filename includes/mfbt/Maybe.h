...
#include "./MaybeStorageBase.h"
#include "./MemoryChecking.h"
#include "./OperatorNewExtensions.h"
#include "./Poison.h"
...
template <size_t Offset, size_t NOffsets> struct InlinePoisoner {
...
template <size_t N> struct InlinePoisoner<N, N> {
...
template <size_t ObjectSize> struct OutOfLinePoisoner {
  static MOZ_NEVER_INLINE void poison(void *p, const uintptr_t) {
    mozWritePoison(p, ObjectSize);
  }
};

template <typename T> inline void PoisonObject(T *p) {
...
template <typename T> struct MaybePoisoner {
...
template <typename T> class Maybe_CopyMove_Enabler<T, true, true, true> {
public:
...
template <typename T> class Maybe_CopyMove_Enabler<T, true, false, true> {
public:
  Maybe_CopyMove_Enabler() = default;
...
template <typename T> class Maybe_CopyMove_Enabler<T, false, true, true> {
public:
  Maybe_CopyMove_Enabler() = default;
...
template <typename T> class Maybe_CopyMove_Enabler<T, false, false, true> {
public:
  Maybe_CopyMove_Enabler() = default;
...
template <typename T> class Maybe_CopyMove_Enabler<T, false, true, false> {
public:
  Maybe_CopyMove_Enabler() = default;
...
template <typename T> struct MaybeStorage<T, false> : MaybeStorageBase<T> {
protected:
...
template <typename T> struct MaybeStorage<T, true> : MaybeStorageBase<T> {
protected:
  char mIsSome = false; // not bool -- guarantees minimal space consumption
...
public:
   ...
   * actually be used.
   */
  template <typename V> constexpr T valueOr(V &&aDefault) const {
    if (isSome()) {
      return ref();
    }
   ...
   */
  template <typename F> constexpr T valueOrFrom(F &&aFunc) const {
    if (isSome()) {
      return ref();
    }
    return aFunc();
   ...
   */
  template <typename F> T *ptrOrFrom(F &&aFunc) {
    if (isSome()) {
      return ptr();
    }
    return aFunc();
  ...
  template <typename F> const T *ptrOrFrom(F &&aFunc) const {
    if (isSome()) {
      return ptr();
    }
    return aFunc();
   ...
   */
  template <typename F> constexpr T &refOrFrom(F &&aFunc) {
    if (isSome()) {
      return ref();
    }
    return aFunc();
  ...
  template <typename F> constexpr const T &refOrFrom(F &&aFunc) const {
    if (isSome()) {
      return ref();
    }
    return aFunc();
   ...
   * this Maybe. */
  template <typename Func> constexpr Maybe &apply(Func &&aFunc) {
    if (isSome()) {
  ...
  template <typename Func> constexpr const Maybe &apply(Func &&aFunc) const {
    if (isSome()) {
      std::forward<Func>(aFunc)(ref());
    }
    return *this;
   ...
   */
  template <typename Func> constexpr auto map(Func &&aFunc) {
    if (isSome()) {
  ...
  template <typename Func> constexpr auto map(Func &&aFunc) const {
    if (isSome()) {
      return Some(std::forward<Func>(aFunc)(ref()));
    }
    return Maybe<decltype(std::forward<Func>(aFunc)(ref()))>{};
   ...
   */
  template <typename... Args> constexpr void emplace(Args &&... aArgs);
...
template <typename T> class Maybe<T &> {
public:
  ...
  template <typename Func> Maybe &apply(Func &&aFunc) {
    if (isSome()) {
      std::forward<Func>(aFunc)(ref());
    }
    return *this;
  ...
  template <typename Func> const Maybe &apply(Func &&aFunc) const {
    if (isSome()) {
      std::forward<Func>(aFunc)(ref());
    }
    return *this;
  ...
  template <typename Func> auto map(Func &&aFunc) {
  ...
  template <typename Func> auto map(Func &&aFunc) const {
    Maybe<decltype(std::forward<Func>(aFunc)(ref()))> val;
    if (isSome()) {
      val.emplace(std::forward<Func>(aFunc)(ref()));
    }
...
template <typename T> constexpr T Maybe<T>::value() const & {
  MOZ_DIAGNOSTIC_ASSERT(isSome());
  return ref();
}

template <typename T> constexpr T Maybe<T>::value() && {
  MOZ_DIAGNOSTIC_ASSERT(isSome());
  return std::move(ref());
}
...
template <typename T> T *Maybe<T>::ptr() {
  MOZ_DIAGNOSTIC_ASSERT(isSome());
  return &ref();
}

template <typename T> constexpr const T *Maybe<T>::ptr() const {
  MOZ_DIAGNOSTIC_ASSERT(isSome());
  return &ref();
}

template <typename T> constexpr T *Maybe<T>::operator->() {
  MOZ_DIAGNOSTIC_ASSERT(isSome());
  return ptr();
}

template <typename T> constexpr const T *Maybe<T>::operator->() const {
  MOZ_DIAGNOSTIC_ASSERT(isSome());
  return ptr();
}

template <typename T> constexpr T &Maybe<T>::ref() & {
  MOZ_DIAGNOSTIC_ASSERT(isSome());
  return mStorage.val;
}

template <typename T> constexpr const T &Maybe<T>::ref() const & {
  MOZ_DIAGNOSTIC_ASSERT(isSome());
  return mStorage.val;
}
 ...
 */
template <typename T, typename U> constexpr Maybe<U> Some(T &&aValue) {
  return {std::forward<T>(aValue), typename Maybe<U>::SomeGuard{}};
}

template <typename T> constexpr Maybe<T &> SomeRef(T &aValue) {
  Maybe<T &> value;
  value.emplace(aValue);
  return value;
}

template <typename T> constexpr Maybe<T &> ToMaybeRef(T *const aPtr) {
...
ImplCycleCollectionTraverse(nsCycleCollectionTraversalCallback &aCallback,
                            mozilla::Maybe<T> &aField, const char *aName,
                            uint32_t aFlags = 0) {
  if (aField) {
