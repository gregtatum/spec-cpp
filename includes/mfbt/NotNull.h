...
template <typename T> class MovingNotNull;
...
template <typename T> class NotNull<T *> {
  template <typename U> friend constexpr NotNull<U> WrapNotNull(U aBasePtr);
  template <typename U>
  friend constexpr NotNull<U *> WrapNotNullUnchecked(U *aBasePtr);
  template <typename U, typename... Args>
...
template <typename T> constexpr NotNull<T> WrapNotNull(T aBasePtr) {
...
template <typename T> constexpr NotNull<T> WrapNotNullUnchecked(T aBasePtr) {
  return NotNull<T>{std::move(aBasePtr)};
}
...
template <typename T> class MOZ_NON_AUTOABLE MovingNotNull {
  template <typename U>
...
template <typename T> constexpr MovingNotNull<T> WrapMovingNotNull(T aBasePtr) {
  MOZ_RELEASE_ASSERT(aBasePtr);
...
template <typename Pointer> struct PointedTo {
...
// pointers.
// See bug 1443367.
template <typename T> struct PointedTo<T *> {
  using Type = T;
  using NonConstType = T;
};

template <typename T> struct PointedTo<const T *> {
  using Type = const T;
  using NonConstType = T;
};
