...
#ifdef MOZ_REFCOUNTED_LEAK_CHECKING
        : mPointer(aPointer), mTypeName(aPointer->typeName())
#endif
    {
    }
...
template <typename T, RefCountAtomicity Atomicity> class RC {
public:
...
template <typename T> class RC<T, AtomicRefCount> {
public:
  explicit RC(T aCount) : mValue(aCount) {}
