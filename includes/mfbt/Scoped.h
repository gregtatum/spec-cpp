 ...
 *   }
 */
template <typename Traits> class MOZ_NON_TEMPORARY_CLASS Scoped {
public:
...
#define MOZ_TYPE_SPECIFIC_SCOPED_POINTER_TEMPLATE(name, Type, Deleter)         \
  template <> inline void TypeSpecificDelete(Type *aValue) {                   \
    Deleter(aValue);                                                           \
  }                                                                            \
...
template <typename T> struct TypeSpecificScopedPointerTraits {
  typedef T *type;
