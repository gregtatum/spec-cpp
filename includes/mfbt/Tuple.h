 ...
 * by grouping them.
 */
template <typename... Ts> struct Group;
...
template <typename Source, typename Target> struct CheckConvertibility;
 ...
 */
struct IgnoreImpl {
  template <typename T> constexpr const IgnoreImpl &operator=(const T &) const {
    return *this;
  }
};
 ...
 * implementation.
 */
template <std::size_t Index, typename... Elements> struct TupleImpl;

/*
 * The base case of the inheritance recursion (and also the implementation
 * of an empty tuple).
 */
template <std::size_t Index> struct TupleImpl<Index> {
  bool operator==(const TupleImpl<Index> &aOther) const { return true; }

  template <typename F> void ForEach(const F &aFunc) {}
};
...
struct TupleImpl<Index, HeadT, TailT...>
  ...
  template <typename F> void ForEach(const F &aFunc) const & {
    aFunc(Head(*this));
...
template <typename F> inline void ForEach(Tuple<> &aTuple, const F &aFunc) {}
