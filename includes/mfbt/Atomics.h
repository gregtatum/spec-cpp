...
template <> struct AtomicOrderConstraints<Relaxed> {
...
template <> struct AtomicOrderConstraints<ReleaseAcquire> {
...
template <> struct AtomicOrderConstraints<SequentiallyConsistent> {
...
template <typename T, MemoryOrdering Order> struct IntrinsicBase {
...
template <typename T> struct ToStorageTypeArgument {
  static constexpr T convert(T aT) { return aT; }
};

template <typename T, MemoryOrdering Order> class AtomicBase {
