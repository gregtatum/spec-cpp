...
template <typename T> struct AllowDeprecatedAbsFixed : std::false_type {};

template <> struct AllowDeprecatedAbsFixed<int32_t> : std::true_type {};
template <> struct AllowDeprecatedAbsFixed<int64_t> : std::true_type {};

template <typename T> struct AllowDeprecatedAbs : AllowDeprecatedAbsFixed<T> {};

template <> struct AllowDeprecatedAbs<int> : std::true_type {};
template <> struct AllowDeprecatedAbs<long> : std::true_type {};
...
template <typename T>
inline std::enable_if_t<detail::AllowDeprecatedAbs<T>::value, T>
...
template <typename T, typename = void> struct AbsReturnType;
...
template <> inline float Abs<float>(const float aFloat) {
  return std::fabs(aFloat);
}

template <> inline double Abs<double>(const double aDouble) {
  return std::fabs(aDouble);
}

template <> inline long double Abs<long double>(const long double aLongDouble) {
...
template <typename T> class CeilingLog2<T, 4> {
public:
...
template <typename T> class CeilingLog2<T, 8> {
public:
  static uint_fast8_t compute(const T aValue) {
    // Check for <= 1 to avoid the == 0 undefined case.
 ...
 */
template <typename T> inline uint_fast8_t CeilingLog2(const T aValue) {
...
template <typename T, size_t Size = sizeof(T)> class FloorLog2;

template <typename T> class FloorLog2<T, 4> {
public:
  static uint_fast8_t compute(const T aValue) {
...
template <typename T> class FloorLog2<T, 8> {
public:
  static uint_fast8_t compute(const T aValue) {
 ...
 */
template <typename T> inline uint_fast8_t FloorLog2(const T aValue) {
 ...
 */
template <typename T> constexpr bool IsPowerOfTwo(T x) {
