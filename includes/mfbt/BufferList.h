...
template <typename AllocPolicy> class BufferList : private AllocPolicy {
  ...
  template <typename OtherAllocPolicy> friend class BufferList;
...
public:
  ...
  BufferList(size_t aInitialSize, size_t aInitialCapacity,
      ...
      : AllocPolicy(aAP), mOwning(true), mSegments(aAP), mSize(0),
  ...
  BufferList(BufferList &&aOther)
      : mOwning(aOther.mOwning), mSegments(std::move(aOther.mSegments)),
        mSize(aOther.mSize), mStandardCapacity(aOther.mStandardCapacity) {
  ...
  Borrow(IterImpl &aIter, size_t aSize, bool *aSuccess,
  ...
  MoveFallible(bool *aSuccess, OtherAllocPolicy aAP = OtherAllocPolicy());
