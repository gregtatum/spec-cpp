...
#include "./ArrayUtils.h" // for PointerRangeSize
 ...
 */
template <typename T> static bool CapacityHasExcessSpace(size_t aCapacity) {
 ...
 */
template <typename T, size_t N, class AP, bool IsPod> struct VectorImpl {
  /*
 ...
 * IsPod.
 */
template <typename T, size_t N, class AP> struct VectorImpl<T, N, AP, true> {
  template <typename... Args>
  MOZ_NONNULL(1)
  static inline void new_(T *aDst, Args &&... aArgs) {
...
class MOZ_NON_PARAM Vector final : private AllocPolicy {
   ...
   */
  template <size_t MinimumInlineCapacity, size_t Dummy> struct ComputeCapacity {
    static constexpr size_t value =
        tl::Min<MinimumInlineCapacity, kMaxInlineBytes / sizeof(T)>::value;
  };

  template <size_t Dummy> struct ComputeCapacity<0, Dummy> {
...
#pragma warning(disable : 4324)
  ...
  template <size_t Dummy> struct CRAndStorage<0, Dummy> : CapacityAndReserved {
    explicit CRAndStorage(size_t aCapacity, size_t aReserved)
        : CapacityAndReserved(aCapacity, aReserved) {}
    CRAndStorage() = default;
...
#pragma warning(pop)
  ...
  template <typename U> void internalMoveAppend(U *aBegin, size_t aLength);
...
public:
   ...
   */
  template <typename... Args>[[nodiscard]] bool emplaceBack(Args &&... aArgs) {
  ...
  template <typename U>[[nodiscard]] bool moveAppend(U *aBegin, U *aEnd);
   ...
   */
  template <typename U>[[nodiscard]] T *insert(T *aP, U &&aVal);
...
MOZ_ALWAYS_INLINE Vector<T, N, AP>::Vector(AP aAP)
    : AP(std::move(aAP)), mLength(0), mTail(kInlineCapacity, 0)
#ifdef DEBUG
      ,
      mEntered(false)
#endif
...
template <typename T, size_t N, class AP>
MOZ_ALWAYS_INLINE bool
Vector<T, N, AP>::internalEnsureCapacity(size_t aNeeded) {
  if (mLength + aNeeded > mTail.mCapacity) {
    if (MOZ_UNLIKELY(!growStorageBy(aNeeded))) {
      return false;
    }
