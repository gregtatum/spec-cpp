 ...
 */
template <typename T> struct LinkedListElementTraits {
  typedef T *RawType;
...
template <typename T> struct LinkedListElementTraits<RefPtr<T>> {
  typedef T *RawType;
  typedef const T *ConstRawType;
...
public:
  template <typename Type, typename Element> class Iterator {
    Type mCurrent;
...
ImplCycleCollectionTraverse(nsCycleCollectionTraversalCallback &aCallback,
                            LinkedList<RefPtr<T>> &aField, const char *aName,
                            uint32_t aFlags = 0) {
  typedef typename detail::LinkedListElementTraits<T> Traits;
  typedef typename Traits::RawType RawType;
...
template <typename T> class AutoCleanLinkedList : public LinkedList<T> {
private:
