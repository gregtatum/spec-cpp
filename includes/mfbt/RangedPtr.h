 ...
 */
template <typename T> class RangedPtr {
  template <typename U> friend class RangedPtr;
...
public:
  ...
  template <typename U> RangedPtr<U> ReinterpretCast() const {
#ifdef DEBUG
   ...
   */
  template <typename U> RangedPtr<T> &operator=(U *aPtr) {
    *this = create(aPtr);
    return *this;
  }

  template <typename U> RangedPtr<T> &operator=(const RangedPtr<U> &aPtr) {
  ...
  template <typename U> bool operator==(const RangedPtr<U> &aOther) const {
  ...
  template <typename U> bool operator==(const U *u) const { return mPtr == u; }
