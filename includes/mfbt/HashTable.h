...
public:
  ...
  template <typename U>[[nodiscard]] bool put(U &&aU) {
  ...
  template <typename U>[[nodiscard]] bool putNew(U &&aU) {
  ...
  //
  template <typename U> void putNewInfallible(const Lookup &aLookup, U &&aU) {
  ...
  template <typename U>[[nodiscard]] bool add(AddPtr &aPtr, U &&aU) {
...
template <typename Key> struct PointerHasher {
  using Lookup = Key;
...
template <class Key, typename> struct DefaultHasher {
  using Lookup = Key;
...
template <class T> struct DefaultHasher<T *> : PointerHasher<T *> {};

// A DefaultHasher specialization for mozilla::UniquePtr.
template <class T, class D> struct DefaultHasher<UniquePtr<T, D>> {
...
template <> struct DefaultHasher<float> {
  using Key = float;
  using Lookup = Key;
...
template <typename HashPolicy> struct FallibleHashMethods {
  ...
  template <typename Lookup> static bool hasHash(Lookup &&aLookup) {
    return true;
  }
  ...
  template <typename Lookup> static bool ensureHash(Lookup &&aLookup) {
    return true;
  }
};
...
template <class T> class EntrySlot {
  using NonConstT = std::remove_const_t<T>;
...
public:
...
#ifdef DEBUG
          ,
          mTable(&aTable), mGeneration(aTable.generation())
#endif
    {
    }
  ...
  class AddPtr : public Ptr {
    ...
    AddPtr(Slot aSlot, const HashTable &aTable, HashNumber aHashNumber)
        : Ptr(aSlot, aTable), mKeyHash(aHashNumber)
#ifdef DEBUG
          ,
    ...
    AddPtr(const HashTable &aTable, HashNumber aHashNumber)
        : Ptr(aTable), mKeyHash(aHashNumber)
#ifdef DEBUG
          ,
          mMutationCount(aTable.mMutationCount)
#endif
...
#ifdef DEBUG
          ,
          mTable(aTable), mMutationCount(aTable.mMutationCount),
          mGeneration(aTable.generation()), mValidEntry(true)
#endif
    {
      if (!done() && !mCur.isLive()) {
        moveToNextLiveEntry();
  ...
  public:
    MOZ_IMPLICIT ModIterator(ModIterator &&aOther)
        : Iterator(aOther), mTable(aOther.mTable), mRekeyed(aOther.mRekeyed),
  ...
  public:
    template <class Map> explicit Enum(Map &map) : mIter(map.mImpl) {}
...
public:
  HashTable(AllocPolicy aAllocPolicy, uint32_t aLen)
      : AllocPolicy(std::move(aAllocPolicy)), mGen(0),
        mHashShift(hashShift(aLen)), mTable(nullptr), mEntryCount(0),
        mRemovedCount(0)
#ifdef DEBUG
        ,
        mMutationCount(0), mEntered(false)
#endif
  {
  }
...
private:
  ...
  RebuildStatus
  changeTableSize(uint32_t newCapacity,
                  FailureBehavior aReportFailure = ReportFailure) {
  ...
  RebuildStatus
  rehashIfOverloaded(FailureBehavior aReportFailure = ReportFailure) {
