...
namespace detail {
template <typename R> auto ResultRefAsParam(R &aResult) { return &aResult; }
...
template <typename T> struct outparam_as_reference;

template <typename T> struct outparam_as_reference<T *> { using type = T &; };
...
auto ToResultInvokeMemberFunction(T &aObj, const Func &aFunc,
                                  Args &&... aArgs) {
...
template <typename T> auto DerefHelper(const T &) -> T &;
