...
template <class T, class D> struct PointerTypeImpl<T, D, false> {
  typedef T *Type;
};

template <class T, class D> struct PointerType {
 ...
 * |UniquePtr&&| argument.
 */
template <typename T, class D> class UniquePtr {
public:
...
template <typename T, class D> class UniquePtr<T[], D> {
public:
  typedef T *Pointer;
  typedef T ElementType;
  typedef D DeleterType;
...
template <typename T> class DefaultDelete<T[]> {
public:
  constexpr DefaultDelete() = default;
  ...
  template <typename U> void operator()(U *aPtr) const = delete;
};
...
template <typename T> struct UniqueSelector {
  typedef UniquePtr<T> SingleObject;
};

template <typename T> struct UniqueSelector<T[]> {
  typedef UniquePtr<T[]> UnknownBound;
};

template <typename T, decltype(sizeof(int)) N> struct UniqueSelector<T[N]> {
