...
template <size_t N, typename... Ts> struct Nth;

template <typename T, typename... Ts> struct Nth<0, T, Ts...> {
  using Type = T;
};

template <size_t N, typename T, typename... Ts> struct Nth<N, T, Ts...> {
  using Type = typename Nth<N - 1, Ts...>::Type;
};

/// SelectVariantTypeHelper is used in the implementation of SelectVariantType.
template <typename T, typename... Variants> struct SelectVariantTypeHelper;

template <typename T> struct SelectVariantTypeHelper<T> {
...
template <typename... Ts> struct VariantTag {
private:
...
struct VariantImplementation<Tag, N, T> {
  template <typename U> static Tag tag() {
  ...
  template <typename Variant> static void destroy(Variant &aV) {
    aV.template as<N>().~T();
  }
 ...
 */
template <typename T> struct AsVariantTemporary {
...
public:
  ...
  template <typename T, typename... Args> T &emplace(Args &&... aTs) {
    Impl::destroy(*this);
    tag = Impl::template tag<T>();
    ::new (KnownNotNull, ptr()) T(std::forward<Args>(aTs)...);
    return as<T>();
  ...
  template <size_t N> const typename detail::Nth<N, Ts...>::Type &as() const & {
    static_assert(N < sizeof...(Ts),
                  "provided an index outside of this Variant's type list");
    MOZ_RELEASE_ASSERT(is<N>());
  ...
  template <typename Matcher> decltype(auto) match(Matcher &&aMatcher) const & {
  ...
  template <typename Matcher> decltype(auto) match(Matcher &&aMatcher) & {
    return Impl::match(std::forward<Matcher>(aMatcher), *this);
  }
