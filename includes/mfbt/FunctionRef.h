...
#include "./OperatorNewExtensions.h" // mozilla::NotNull, ::operator new
...
using EnableMatchingFunction =
    std::enable_if_t<CompatibleReturnType<decltype(std::declval<Func &>()(
                                              std::declval<Params>()...)),
                                          Ret>::value,
                     int>;
...
struct GetCallableTag {
  ...
  template <typename T>
  static MatchingFunctionPointerTag
  test(int, T &obj, EnableMatchingFunction<decltype(+obj), Ret, Params...> = 0);
