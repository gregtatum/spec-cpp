 ...
 * reference instead.
 */
template <class T1, class T2> class MOZ_NON_PARAM MaybeOneOf {
...
public:
  ...
  template <class T> bool constructed() const {
    return state == Type2State<T>::result;
  }

  template <class T, class... Args> void construct(Args &&... aArgs) {
    MOZ_ASSERT(state == None);
    state = Type2State<T>::result;
    ::new (KnownNotNull, data()) T(std::forward<Args>(aArgs)...);
  }

  template <class T> T &ref() { return as<T>(); }

  template <class T> const T &ref() const { return as<T>(); }
