...
template <> struct FloatingPointTrait<float> {
protected:
  using Bits = uint32_t;
...
template <> struct FloatingPointTrait<double> {
protected:
  using Bits = uint64_t;
...
template <typename T> static MOZ_ALWAYS_INLINE bool IsNaN(T aValue) {
  /*
...
template <typename T> static MOZ_ALWAYS_INLINE bool IsInfinite(T aValue) {
...
template <typename T> static MOZ_ALWAYS_INLINE bool IsFinite(T aValue) {
  /*
...
template <typename T> static MOZ_ALWAYS_INLINE bool IsNegativeZero(T aValue) {
...
template <typename T> static MOZ_ALWAYS_INLINE bool IsPositiveZero(T aValue) {
 ...
 */
template <typename T> static MOZ_ALWAYS_INLINE T ToZeroIfNonfinite(T aValue) {
...
/** Returns +Infinity. */
template <typename T> static MOZ_ALWAYS_INLINE T PositiveInfinity() {
  /*
...
/** Returns -Infinity. */
template <typename T> static MOZ_ALWAYS_INLINE T NegativeInfinity() {
  /*
 ...
 */
template <typename T, int SignBit> struct InfinityBits {
...
SpecificNaN(int signbit, typename FloatingPoint<T>::Bits significand,
  ...
  BitwiseCast<T>((signbit ? Traits::kSignBit : 0) | Traits::kExponentBits |
                     significand,
                 result);
...
template <typename T> static MOZ_ALWAYS_INLINE T MinNumberValue() {
  typedef FloatingPoint<T> Traits;
  typedef typename Traits::Bits Bits;
 ...
 */
template <typename T> static MOZ_ALWAYS_INLINE T UnspecifiedNaN() {
  /*
 ...
 * zero) or both NaN.
 */
template <typename T> static inline bool EqualOrBothNaN(T aValue1, T aValue2) {
  if (IsNaN(aValue1)) {
    return IsNaN(aValue2);
  }
 ...
 */
template <typename T> static inline T NaNSafeMin(T aValue1, T aValue2) {
 ...
 * |aValue1| and |aValue2| otherwise.
 */
template <typename T> static inline T NaNSafeMax(T aValue1, T aValue2) {
  if (IsNaN(aValue1) || IsNaN(aValue2)) {
    return UnspecifiedNaN<T>();
  }
