...
void run_tests() {
  test::suite("features::lvalueReference", []() {
    ...
    test::describe("const lvalue references", []() {
      ...
      test::equal(value, 33, "The int value has changed");
      test::equal(refValue, 33, "The ref value will change even thought it's const");
    ...
    test::describe("lvalue references can be stored in structs", []() {
      ...
      test::equal(refIntHolder.value, 33, "The ref struct's value can be changed.");
      test::equal(intHolder.value, 32, "The non-ref struct's value remains the same.");
    ...
    test::describe("lvalue references in structs can't break out to outer lexical scopes", []() {
      ...
      test::equal(refIntHolder.value, 32, "The ref struct can be initialized with a value.");
      test::equal(intHolder.value, 32, "The struct  can be initialized with a value.");

      {
        int valueInside = 33;
        intHolder = { valueInside };
        test::equal(intHolder.value, 33, "Setting the intHolder in a different lexical scope is fine, as the value is copied.");
        ...
        test::ok(true, "We can't assign a reference to a value in a exterior lexical scope block");
      }
