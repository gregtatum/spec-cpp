...
void run_tests() {
  test::suite("features::arrays", []() {
    test::describe("arrays can be created on the stack.", []() {
      int array [4];
      test::ok(bool(array), "An array was created, but it's still uninitialized. Acces");
      test::ok(isInt(array[0]), "The value can be accessed, but its value could be anything. UNDEFINED BEHAVIOR");
      array[0] = 11;
      array[1] = 22;
      array[2] = 33;
      array[3] = 44;
      ...
      int index = 7;
      test::ok(isInt(array[index]), "The compiler doesn't care if we unintentionally read values outside of the array. UNDEFINED BEHAVIOR");
    });
    ...
    test::describe("Pointer arithmetic works on arrays coerced to pointers", []() {
      ...
      test::equal(*firstElement, 11, "Can read the first element as a pointer");
      test::equal(*(firstElement + 1), 22, "Can read the second element using pointer arithmetic");
      test::equal(*(firstElement + 2), 33, "Can read the second element using pointer arithmetic");
    });
    ...
    test::describe("You can pass by references, but only at a fixed array size", []() {
      ...
      test::equal(array[1], 44, "Second element was modified because the function call returned an lvalue reference");
